...
dh__normalize_and_verify_image_name_(){
  local NAME=${1}
  local REPO TAG
  [[ -n ${NAME} ]] || return
  ...
  dh_list_all_images | grep -q "^${NAME}$" || return
  echo ${NAME}
}

dh_list_all_containers_by_name(){
  local LIST=$(docker ps --all --quiet --no-trunc)
  [[ -n ${LIST} ]] || return
  docker inspect --format='{{.Name}}' ${LIST} | sed 's#^/##'
}

dh_list_running_containers_by_name(){
  local LIST=$(docker ps --quiet --no-trunc)
  [[ -n ${LIST} ]] || return
  docker inspect --format='{{.Name}}' ${LIST} | sed 's#^/##'
}

dh_list_non_running_containers_by_name(){
  local A B
  while read A
  do
    while read B
    do
      [[ ${A} == ${B} ]] && continue 2
    done < <(dh_list_running_containers_by_name)
    echo ${A}
